...
func TmpfileCopy(filename string) (fname2 string, err error) {
	fname2 = Tmpfile()
	_, err = copy(filename, fname2)
	return
}
...
func Fade(fname string, fadeIn float64, fadeOut float64) (fname2 string, err error) {
	fname2 = Tmpfile()
...
func LoopCrossfade(fname string, fadetime float64) (fname2 string, err error) {
	...
	fname2, err = Fade(fname, fadetime, fadetime)
	if err != nil {
		return
	}

	mainPart, err := Trim(fname2, 0, totaltime-fadetime)
	if err != nil {
		return
	}
...
// SilenceTrim trims silence around a file
func SilenceTrimDB(fname string, silenceTime float64, dB int) (fname2 string, err error) {
	fname2 = Tmpfile()
